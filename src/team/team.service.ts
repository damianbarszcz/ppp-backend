...
export class TeamService {
      ...
      if (memberId !== senderId) {
        try {
          const existingInvitation =
            await this.teamInvitationRepository.findOne({
              where: {
                team_id: teamId,
          ...
          console.error(
            `Błąd tworzenia zaproszenia dla użytkownika ${memberId}:`,
            error,
          );
        }
      }
    ...
    for (const memberId of memberIds) {
      if (memberId !== senderId) {
        ...
        try {
          ...
          console.error(
            `Błąd wysyłania powiadomienia do użytkownika ${memberId}:`,
            error,
          );
        }
      }
  ...
  public async getTeamsByUserId(userId: number): Promise<Team[]> {
    ...
    const joinedTeams = acceptedInvitations.map(
  ...
  public async getTeamInvitationsForUser(
    userId: number,
  ): Promise<{
  ...
  public async rejectTeamInvitation(
    invitationId: number,
    userId: number,
  ): Promise<void> {
    const invitation = await this.teamInvitationRepository.findOne({
