...
export class TeamController {
  ...
  public async createTeam(
    ...
    try {
      ...
      const existingTeam = await this.teamService.checkTeamExists(
        dto.title.trim(),
        dto.user_id,
      );
      if (existingTeam) {
        return res.status(HttpStatus.CONFLICT).json({
          success: false,
          errors: [
            { field: 'title', message: 'Zespół o tej nazwie już istnieje' },
          ],
        });
      }
      ...
      return res
        ...
        .json({ success: true, message: 'Zespół został utworzony pomyślnie' });
    } catch (error) {
      return res
  ...
  public async getTeamInvitations(
    ...
    const invitations = await this.teamService.getTeamInvitationsForUser(
      Number(userId),
    );
  ...
  public async acceptTeamInvitation(
    ...
    try {
      await this.teamService.acceptTeamInvitation(
        Number(invitationId),
        body.user_id,
      );
  ...
  async rejectTeamInvitation(
    ...
    try {
      await this.teamService.rejectTeamInvitation(
        Number(invitationId),
        body.user_id,
      );
      return res.status(HttpStatus.OK).json({ success: true });
