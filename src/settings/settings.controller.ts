...
export class SettingsController {
  ...
  async updateName(@Body() body: NameDto, @Res() res: Response) {
    try {
      await this.settingsService.updateName(
        body.name,
        body.surname,
        body.user_id,
      );
      ...
      return res.status(HttpStatus.BAD_REQUEST).json({
        success: false,
        message:
          error.message || 'Błąd podczas aktualizacji imienia i nazwiska',
      });
    }
  }
  ...
  @UsePipes(new ValidationPipe({ transform: true, whitelist: true }))
  async updateUsername(@Body() body: UsernameDto, @Res() res: Response) {
    try {
  ...
  @UsePipes(new ValidationPipe({ transform: true, whitelist: true }))
  async updateBiogram(@Body() body: BiogramDto, @Res() res: Response) {
    try {
  ...
  @UsePipes(new ValidationPipe({ transform: true, whitelist: true }))
  async changePassword(@Body() body: PasswordDto, @Res() res: Response) {
    try {
      const isCurrentPasswordValid =
        await this.settingsService.validateCurrentPassword(
          body.user_id,
  ...
  async updateMentorSubscribePrice(
    ...
    try {
      await this.settingsService.updateMentorSubscribePrice(
        body.mentor_subscribe_price,
        body.user_id,
      );
