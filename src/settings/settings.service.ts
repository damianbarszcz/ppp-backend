...
export class SettingsService {
    ...
    async updateName(name: string, surname: string, user_id: number): Promise<User> {
        try {
    ...
    async updateUsername(username: string, user_id: number): Promise<User> {
        try {
            ...
            if (existingUserWithUsername && existingUserWithUsername.user.id !== user_id) {
    ...
    async checkUsernameExists(username: string, excludeUserId?: number): Promise<boolean> {
        try {
            const query = this.profileRepository.createQueryBuilder('profile')
                ...
                .where('LOWER(profile.username) = LOWER(:username)', { username: username.trim() });
