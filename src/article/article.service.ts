...
export class ArticleService {
  ...
  public async createArticle(ArticleDto: ArticleDto): Promise<Article> {
    const { title, summary, content, content_type, thumbnail_url, user_id } =
      ArticleDto;
  ...
  private async notifyFollowersAboutNewArticle(
    mentorId: number,
    articleTitle: string,
  ): Promise<void> {
    try {
  ...
  private async checkTitleExists(
    ...
    if (existingArticle) {
      throw new ConflictException({
        success: false,
        errors: [
          { field: 'title', message: 'Artykuł o takim tytule już istnieje.' },
        ],
      });
    }
  }
  ...
  private generateSlug(title: string): string {
    ...
    const truncated_slug =
      base_slug.length > max_base_length
