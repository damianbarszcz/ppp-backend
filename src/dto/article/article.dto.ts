import {IsNotEmpty, IsNumber, IsEnum, Length, IsUrl, Matches} from 'class-validator';
...
export class ArticleDto {
    ...
    @Length(500, undefined,  { message: 'Treść artykułu musi mieć minimum 500 znaków.' })
    ...
    @Matches(/^https:\/\/.+\.(jpg|jpeg|png|gif|webp)(\?.*)?$/i, {
        message: 'URL grafiki musi być w formacie https:// i zawierać rozszerzenie obrazu (jpg, jpeg, png, gif, webp).'
    })
    @IsUrl({}, { message: 'URL grafiki musi być prawidłowym adresem URL.' })
    @IsNotEmpty({ message: 'URL grafiki jest wymagane.' })
    thumbnail_url?: string;

    @IsEnum(['free', 'paid'], { message: 'Typ treści może być tylko "free" lub "paid".' })
